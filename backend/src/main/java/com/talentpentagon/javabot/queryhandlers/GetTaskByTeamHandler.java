...
public class GetTaskByTeamHandler implements GetCommand<Integer, List<TaskItem>> {
    ...
    public ResponseEntity<List<TaskItem>> execute(Integer teamId, String sortBy, String status, Integer priority) {
        ...
        List<TaskItem> tasks = teamService.getTeamTasks(persistedTeam.getId(), sortBy, status, priority); // ask Diego
        ...
        List<TaskItem> tasks_2 = taskService.getTasksForTeam(teamId, sortBy, status);
        return ResponseEntity.ok(tasks_2);
