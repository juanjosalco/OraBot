...
if test -z "$TALENT_PENTAGON_LOCATION"; then
  echo "ERROR: this script requires TALENT_PENTAGON_LOCATION to be set"
  exit
fi
...
while ! state_done USER_OCID; do
  ...
  # Validate
  if test ""`oci iam user get --user-id "$USER_OCID" --query 'data."lifecycle-state"' --raw-output 2>$TALENT_PENTAGON_LOG/user_ocid_err` == 'ACTIVE'; then
    state_set USER_OCID "$USER_OCID"
  else
    echo "That user OCID could not be validated"
    cat $TALENT_PENTAGON_LOG/user_ocid_err
  fi
done
...
# Generate Unique Key
while ! state_done OB_KEY; do
  state_set OB_KEY $(python "$TALENT_PENTAGON_LOCATION/utils/python-scripts/generate-unique-key.py")
done
...
while ! state_done COMPARTMENT_OCID; do
  if test $(state_get RUN_TYPE) -ne 3; then
    ...
    if test "$COMPARTMENT_OCID" != "" && test `oci iam compartment get --compartment-id "$COMPARTMENT_OCID" --query 'data."lifecycle-state"' --raw-output 2>/dev/null` == 'ACTIVE'; then
      ...
      COMPARTMENT_OCID=`oci iam compartment create --compartment-id "$(state_get TENANCY_OCID)" --name "$(state_get RUN_NAME)" --description "talent-pentagon" --query 'data.id' --raw-output`
    fi
  fi
...
if ! state_get JAVA_BUILDS; then
  if ps -ef | grep "$TALENT_PENTAGON_LOCATION/utils/java-builds.sh" | grep -v grep; then
    echo "$TALENT_PENTAGON_LOCATION/utils/java-builds.sh is already running"
  else
    echo "Executing java-builds.sh in the background"
    nohup $TALENT_PENTAGON_LOCATION/utils/java-builds.sh &>> $TALENT_PENTAGON_LOG/java-builds.log &
  fi
fi
...
while ! state_done DOCKER_REGISTRY; do
  if test $(state_get RUN_TYPE) -ne 3; then
    ...
    if ! TOKEN=`oci iam auth-token create  --user-id "$(state_get USER_OCID)" --description 'ob docker login' --query 'data.token' --raw-output 2>$TALENT_PENTAGON_LOG/docker_registry_err`; then
      if grep UserCapacityExceeded $TALENT_PENTAGON_LOG/docker_registry_err >/dev/null; then
        ...
        cat $TALENT_PENTAGON_LOG/docker_registry_err
        exit
      fi
      sleep 5
    fi
...
if ! state_get OKE_SETUP; then
  if ps -ef | grep "$TALENT_PENTAGON_LOCATION/utils/oke-setup.sh" | grep -v grep; then
    echo "$TALENT_PENTAGON_LOCATION/utils/oke-setup.sh is already running"
  else
    echo "Executing oke-setup.sh in the background"
    nohup $TALENT_PENTAGON_LOCATION/utils/oke-setup.sh &>>$TALENT_PENTAGON_LOG/oke-setup.log &
  fi
fi

# run db-setup.sh in background
if ! state_get DB_SETUP; then
  if ps -ef | grep "$TALENT_PENTAGON_LOCATION/utils/db-setup.sh" | grep -v grep; then
    echo "$TALENT_PENTAGON_LOCATION/utils/db-setup.sh is already running"
  else
    echo "Executing db-setup.sh in the background"
    nohup $TALENT_PENTAGON_LOCATION/utils/db-setup.sh &>>$TALENT_PENTAGON_LOG/db-setup.log &
  fi
fi
...
if ! state_done PROVISIONING; then
  ...
  while ! state_done PROVISIONING; do
    LOGLINE=`tail -1 $TALENT_PENTAGON_LOG/terraform.log`
...
while ! state_done OB_DB_OCID; do
  ...
  if [[ "$OB_DB_OCID" =~ ocid1.autonomousdatabase* ]]; then
    ...
    echo "ERROR: Incorrect Order DB OCID: $OB_DB_OCID"
    exit
  fi
done
...
if ! state_done OKE_NAMESPACE; then
  echo "`date`: Waiting for kubectl configuration and talent-pentagon namespace"
  while ! state_done OKE_NAMESPACE; do
    LOGLINE=`tail -1 $TALENT_PENTAGON_LOG/state.log`
    echo -ne r"\033[2K\r${LOGLINE:0:120}"
    sleep 2
  done
  echo
...
while ! state_done DB_PASSWORD; do
  ...
  while true; do
    if kubectl create -n talent-pentagon -f -; then
...
ps -ef | grep "$TALENT_PENTAGON_LOCATION/utils" | grep -v grep
...
while ! state_done SETUP_VERIFIED; do
  ...
  if test "$NOT_DONE" -gt 0; then
    # echo "Log files are located in $TALENT_PENTAGON_LOG"
    bgs=$bg_not_done
